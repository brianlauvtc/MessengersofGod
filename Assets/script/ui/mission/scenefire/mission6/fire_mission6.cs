...
public class fire_mission6 : MonoBehaviour
{
    public bool accept;
    public bool done;
    ...
    void Update()
    {   
        accept = player.scenefire_m6_accept;
        done = player.scenefire_m6;
        if (player.scenefire_m6 == true)
        {
            tp.SetActive(true);
        ...
        if (accept == true)
        {
            missionlable.text = "Kill "+ maxkillcount + " Fire Pig : ( " + killcount + " / " + maxkillcount + " )\n" + "Destroy Crystal : ( " + killcount2 + " / " + maxkillcount2 + " )";
            acceptbutton.SetActive(false);
            if (killcount == maxkillcount && killcount2 == maxkillcount2)
            {
                submitbutton.SetActive(true);
                if (player.scenefire_m6 == false) { question.SetActive(true); }
            }
