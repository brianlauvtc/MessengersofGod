...
public class ice_mission2 : MonoBehaviour
{
    public bool accept;
    public bool done;
    ...
    void Update()
    {
        accept = player.sceneice_m2_accept;
        done = player.sceneice_m2;

        if (accept == true)
        {
            acceptbutton.SetActive(false);
            missionlable.text = "Kill " + maxkillcount + " Ice Goblin : ( " + killcount + " / " + maxkillcount + " )";

            if (killcount == maxkillcount)
            {
                submitbutton.SetActive(true);
                if (player.sceneice_m2 == false)
