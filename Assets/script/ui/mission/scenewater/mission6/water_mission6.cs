...
public class water_mission6 : MonoBehaviour
{
    public bool accept;
    public bool done;
    ...
    void Update()
    {   
        accept = player.scenewater_m6_accept;
        done = player.scenewater_m6;

        if (accept == true)
        {
            missionlable.text = "Kill Water Cow : ( " + killcount + " / " + maxkillcount + " )\n" + "Destroy Crystal : ( " + killcount2 + " / " + maxkillcount2 + " )";
            acceptbutton.SetActive(false);
            if (killcount == maxkillcount && killcount2 == maxkillcount2)
            {
                submitbutton.SetActive(true);
                if (player.scenewater_m6 == false)
