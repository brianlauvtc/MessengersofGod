...
public class water_mission2 : MonoBehaviour
{
    public bool accept;
    public bool done;
    ...
    void Update()
    {
        accept = player.scenewater_m2_accept;
        done = player.scenewater_m2;

        if (accept == true)
        {
            acceptbutton.SetActive(false);
            missionlable.text = "Kill " + maxkillcount + " Water Spirit : ( " + killcount + " / " + maxkillcount + " )";

            if (killcount == maxkillcount)
            {
                submitbutton.SetActive(true);
                if (player.scenewater_m2 == false)
