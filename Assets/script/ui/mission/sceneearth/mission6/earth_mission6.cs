...
public class earth_mission6 : MonoBehaviour
{
    public bool accept;
    public bool done;
    ...
    void Update()
    {   
        accept = player.sceneearth_m6_accept;
        done = player.sceneearth_m6;

        if (accept == true)
        {
            missionlable.text = "Kill "+ maxkillcount + " Bull Demon King : ( " + killcount + " / " + maxkillcount + " )\n" + "Destroy Crystal : ( " + killcount2 + " / " + maxkillcount2 + " )";
            acceptbutton.SetActive(false);
            if (killcount == maxkillcount && killcount2 == maxkillcount2)
            {
                submitbutton.SetActive(true);
                if (player.sceneearth_m6 == false)
